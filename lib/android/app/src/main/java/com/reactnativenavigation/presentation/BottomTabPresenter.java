    ...
    public void applyOptions() {
        bottomTabs.perform(bottomTabs -> {
            for (int i = 0; i < tabs.size(); i++) {
                ...
                Float inactiveSize = tab.fontSize.hasValue() ? Float.valueOf(tab.fontSize.get()) : null;
                if (inactiveSize != null){
                    bottomTabs.setTitleInactiveTextSizeInSp(i, inactiveSize);
                }

                Float activeSize = tab.selectedFontSize.hasValue() ? Float.valueOf(tab.selectedFontSize.get()) : null;
                if (activeSize != null){
                    bottomTabs.setTitleActiveTextSizeInSp(i, activeSize);
                }
